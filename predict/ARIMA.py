...
data = pd.read_csv('./601988.SH.csv', dtype={'close': 'float64'}, header=0)
...
# 计算训练集的一阶差分
training_set['diff_1'] = training_set['close'].diff()  # 不使用.dropna()以保留NaN值
training_set['diff_1'] = pd.to_numeric(training_set['diff_1'], errors='coerce')

# 处理 NaN 值，这里我们选择填充 NaN 值为 0
training_set['diff_1'].fillna(method='ffill', inplace=True) 
# 重置训练集的索引
...
# 找出 'trade_date_parsed' 列中值为 NaT 的行
na_rows = df[df['trade_date'].isna()]

# 打印出不符合日期格式的数据和它们所在的行
print(na_rows)
