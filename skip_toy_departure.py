...
class SkipToy1(gym.Env):
    ...
    def __init__(self, num_trains=6, num_stations=6, num_time =60,  pass_od = None, render_mode="console"):
        ...
        self.dwell_time = [40, 50, 60, 70, 80, 90, 100, 110, 120, 130,140,150]
        ...
        self.missing_interval = self.dwell_time[len(self.dwell_time) // 2 +2] *2
        ...
        self.ini_state = [0.00001] * state_length
        
        max_value = [10] * state_length
    ...
    def step(self, action):
        ...
        #the passenger waiting as the current station p^n_ks
         ...
         #calculate the total passenger who waiting at the station
        ...
        if self.station_index < self.station_num -1 :
            ...
            new_action, penatly = self.FirstStation()
        ...
        reward = -sum([(num // self.missing_interval) ** 2 for num in self.TotalWaitingTime])/(len(self.TotalWaitingTime))+ penatly
        ...
        state = self.z_score_normalization(state)
    ...
    # 从每个子列表中取出第一个元素，减去3，然后添加到结果列表B中
    ...
    #    ArrivingTime = [value[0] for value in self.onboard_dict[self.train_index]]
        ...
        if any(x < 0 for x in self.TotalWaitingTime):
            a=3
    ...
    def UpdateTrain(self):
        ...
        if self.station_index == self.station_num-2 and self.train_index < self.train_num-1:
            ...
            self.TotalTravelingTime.extend([self.arrive_recode[self.train_index][self.station_index+1]- item[0]  for item in  self.onboard_dict[self.train_index]])
    ...
    def FirstStation(self):
            ...
            self.arrive_recode[self.train_index][self.station_index] = self.departure_recode[self.train_index][self.station_index]  = self.departure_recode[self.train_index-1][self.station_index] 
            
            #take the passenger in the first station
            
            new_action, penaty = self.ActionDetection()
            
            if new_action:
            
                self.arrive_recode[self.train_index][self.station_index] += new_action
